...
/*   Updated: 2016/10/25 12:45:04 by hcorrale         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_step(t_var *v)
{
	...
	{
		v->w->stepx = -1;
		v->w->sidedistx = (v->w->rayposx - (double)v->w->mapx) *
			v->w->deltadistx;
	}
	else
	{
		v->w->stepx = 1;
		v->w->sidedistx = ((double)v->w->mapx + 1.0 - v->w->rayposx) *
			v->w->deltadistx;
	}
	if (v->w->raydiry < 0)
	{
		v->w->stepy = -1;
		v->w->sidedisty = (v->w->rayposy - (double)v->w->mapy) *
			v->w->deltadisty;
	}
	else
	{
		v->w->stepy = 1;
		v->w->sidedisty = ((double)v->w->mapy + 1.0 - v->w->rayposy) *
			v->w->deltadisty;
	}
}
...
{
	if (v->w->side == 0)
		v->w->perpwalldist = fabs((v->w->mapx - v->w->rayposx +
					(1 - v->w->stepx) / 2) / v->w->raydirx);
	else
		v->w->perpwalldist = fabs((v->w->mapy - v->w->rayposy +
					(1 - v->w->stepy) / 2) / v->w->raydiry);
